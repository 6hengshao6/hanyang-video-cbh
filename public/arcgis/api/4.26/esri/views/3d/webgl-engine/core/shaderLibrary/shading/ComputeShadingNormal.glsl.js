...
}`);break;default:k.neverReached(a.doubleSidedMode);case f.NormalsDoubleSidedMode.COUNT:}switch(a.normalType){case d.NormalAttributeType.Attribute:case d.NormalAttributeType.CompressedAttribute:e.include(l.VertexNormal,a);c.code.add(b.glsl`vec3 shadingNormalWorld() {
return _adjustDoublesided(normalize(vNormalWorld));
}
vec3 shadingNormal_view() {
vec3 normal = normalize(vNormalView);
return gl_FrontFacing ? normal : -normal;
}`);break;case d.NormalAttributeType.ScreenDerivative:e.include(g.VertexPosition,a);c.code.add(b.glsl`vec3 shadingNormalWorld() {
...
}`);break;case d.NormalAttributeType.Ground:a.spherical?(e.include(g.VertexPosition,a),c.code.add(b.glsl`vec3 shadingNormalWorld() {
return normalize(positionWorld());
}`)):c.code.add(b.glsl`vec3 shadingNormalWorld() {
return vec3(0.0, 0.0, 1.0);
}`);c.code.add(b.glsl`vec3 shadingNormal_view() {
return normalize(cross(dFdx(vPosition_view),dFdy(vPosition_view))).xyz;
}`);break;default:k.neverReached(a.normalType);case d.NormalAttributeType.COUNT:}};Object.defineProperty(h,Symbol.toStringTag,{value:"Module"})});