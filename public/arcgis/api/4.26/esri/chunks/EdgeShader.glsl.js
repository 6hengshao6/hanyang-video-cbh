...
function(m,g,r,h,t,u,v,w,x,c,y,z,e,A,B,C,k,D,E,F){function n(b){const a=new y.ShaderBuilder,{vertex:f,fragment:l}=a;b.legacy&&f.uniforms.add([new p("model"),new p("localView")]);a.include(A.AdjustProjectedPosition,b);a.include(k.EdgeUtil,b);a.include(D.LineAmplitude,b);a.include(F.UnpackAttributes,b);a.include(E.LineOffset,b);a.include(t.SliceDraw,b);a.include(C.DiscardNonSilhouetteEdges,b);a.include(B.DiscardByCoverage,b);a.include(u.multipassTerrainTest,b);a.varyings.add("vColor","vec4");a.varyings.add("vRadius",
"float");a.varyings.add("vPosition","vec3");a.varyings.add("vWorldPosition","vec3");a.varyings.add("vViewPos","vec3");a.varyings.add("vLineLengthPixels","float");a.varyings.add("vSizeFalloffFactor","float");f.uniforms.add([new v.Float2PassUniform("pixelToNDC",(q,d)=>r.set(G,2/d.camera.fullViewport[2],2/d.camera.fullViewport[3])),new w.Float4PassUniform("viewport",(q,d)=>d.camera.fullViewport),new x.FloatPassUniform("pixelRatio",(q,d)=>d.camera.pixelRatio)]);a.attributes.add(e.VertexAttribute.POSITION0,
"vec3");a.attributes.add(e.VertexAttribute.POSITION1,"vec3");a.attributes.add(e.VertexAttribute.VARIANTOFFSET,"float");a.attributes.add(e.VertexAttribute.VARIANTSTROKE,"float");a.attributes.add(e.VertexAttribute.VARIANTEXTENSION,"float");f.code.add(c.glsl`
    ...
    void calculateGeometricOutputs(vec3 viewPosV0, vec3 viewPosV1, vec3 worldPosV0, vec3 worldPosV1, vec3 worldNormal, UnpackedAttributes unpackedAttributes) {
      ...
      ${b.mode===k.EdgeUtilMode.SKETCH?c.glsl`
        if (lineLengthPixels <= 3.0) {
          gl_Position = vec4(10.0, 10.0, 10.0, 1.0);
          return;
        }`:b.mode===k.EdgeUtilMode.MIXED?c.glsl`
  ...
  `);return a}const G=h.create();let p=function(b){function a(l){return f.call(this,l,"mat4")}g._inherits(a,b);var f=g._createSuper(a);return g._createClass(a)}(z.Uniform);h=Object.freeze(Object.defineProperty({__proto__:null,build:n},Symbol.toStringTag,{value:"Module"}));m.EdgeShader=h;m.build=n});