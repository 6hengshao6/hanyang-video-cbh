...
a.pbrMode===g.PBRMode.Schematic)c.include(g.PhysicallyBasedRenderingParameters,a),a.hasNormalTexture&&c.include(P.ComputeNormalTexture,a);const E=a.output===e.ShaderOutput.Shadow||a.output===e.ShaderOutput.ShadowHighlight||a.output===e.ShaderOutput.ShadowExcludeHighlight;E&&a.componentData===x.ComponentDataType.Varying?h.code.add(b.glsl`#define discardShadows(castShadows) { if(!castShadows) { gl_Position = vec4(1e38, 1e38, 1e38, 1.0); return; } }`):h.code.add(b.glsl`#define discardShadows(castShadows) {}`);
...
float shadow = lightingGlobalFactor * (1.0 - additionalAmbientScale);`)):d.code.add(b.glsl`float shadow = 0.0;`),l&&d.uniforms.add(new t.Texture2DPassUniform("ovNormalTex",(m,k)=>u(k))),a.snowCover&&d.code.add(b.glsl`vec3 surfaceNormal = normalize(cross(dFdx(vPositionWorldCameraRelative), dFdy(vPositionWorldCameraRelative)));
float snow = smoothstep(0.5, 0.55, dot(surfaceNormal, normalize(positionWorld())));
materialColor.rgb = mix(materialColor.rgb, vec3(1), snow);`),d.code.add(b.glsl`
      ...
      `)),d.code.add(b.glsl`
        ...
        ${a.transparencyPassType===W.TransparencyPassType.Color?"fragColor \x3d premultiplyAlpha(fragColor);":""}
      }
