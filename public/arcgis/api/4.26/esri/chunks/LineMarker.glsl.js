...
function(q,l,r,e,x,y,z,A,B,C,D,E,t,F,u,v,c,G,H,I,J,m,n){function w(a){const b=new H.ShaderBuilder,p=a.hasMultipassTerrain&&(a.output===e.ShaderOutput.Color||a.output===e.ShaderOutput.Alpha),h=a.space===n.LineMarkerSpace.World;b.include(y.RibbonVertexPosition,a);b.include(A.MarkerSizing,a);a.output===e.ShaderOutput.Depth&&b.include(z.OutputDepth,a);const {vertex:d,fragment:k}=b;k.include(E.RgbaFloatEncoding);t.addProjViewLocalOrigin(d,a);b.attributes.add(m.VertexAttribute.POSITION,"vec3");b.attributes.add(m.VertexAttribute.UV0,
"vec2");b.attributes.add(m.VertexAttribute.AUXPOS1,"vec3");b.varyings.add("vColor","vec4");b.varyings.add("vpos","vec3");b.varyings.add("vUV","vec2");b.varyings.add("vSize","float");r.addLinearDepth(b);p&&b.varyings.add("depth","float");a.hasTip&&b.varyings.add("vLineWidth","float");d.uniforms.add([new F.Float2PassUniform("nearFar",(g,f)=>f.camera.nearFar),new u.Float4PassUniform("viewport",(g,f)=>f.camera.fullViewport)]);d.code.add(c.glsl`vec4 projectAndScale(vec4 pos) {
...
}`);h?(b.attributes.add(m.VertexAttribute.NORMAL,"vec3"),t.addViewNormal(d),d.constants.add("tiltThreshold","float",.7),d.code.add(c.glsl`vec3 perpendicular(vec3 v) {
...
return vec2(v.y, -v.x);
    ...
    `);a.space===n.LineMarkerSpace.Screen&&(d.uniforms.add(new G.Matrix4PassUniform("inverseProjectionMatrix",(g,f)=>f.camera.inverseProjectionMatrix)),d.code.add(c.glsl`vec3 inverseProject(vec4 posScreen) {
...
return abs(cos) > 0.001 && t > 0.0;
}`),d.uniforms.add(new v.FloatPassUniform("perScreenPixelRatio",(g,f)=>f.camera.perScreenPixelRatio)),d.code.add(c.glsl`
  ...
  `));d.uniforms.add(new v.FloatPassUniform("pixelRatio",(g,f)=>f.camera.pixelRatio));r.addCalculateLinearDepth(b);d.code.add(c.glsl`void main(void) {
if (uv0.y == 0.0) {
...
clip(pos, prev);`);h?(a.hideOnShortSegments&&d.code.add(c.glsl`if (areWorldMarkersHidden(pos, prev)) {
gl_Position = vec4(1e038, 1e038, 1e038, 1.0);
return;
...
displacedPosScreen = projectAndScale(pos);`));d.code.add(c.glsl`
  ...
  `);p&&b.include(B.multipassTerrainTest,a);b.include(x.SliceDraw,a);k.uniforms.add([new u.Float4PassUniform("intrinsicColor",g=>g.color),new I.Texture2DPassUniform("tex",g=>g.texture)]);k.include(D.ColorConversion);b.constants.add("texelSize","float",1/l.MARKER_TEXTURE_SIZE);k.code.add(c.glsl`float markerAlpha(vec2 samplePos) {
