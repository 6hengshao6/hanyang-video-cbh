...
function(p,D,v,g,E,F,G,H,w,I,J,K,q,L,M,N,r,l,a,O,P,Q,m,R,x){function y(b){const c=new P.ShaderBuilder,{vertex:d,fragment:f}=c,n=b.hasMultipassTerrain&&(b.output===g.ShaderOutput.Color||b.output===g.ShaderOutput.Alpha);c.include(K.PiUtils);c.include(G.RibbonVertexPosition,b);c.include(w.LineStipple,b);const z=b.applyMarkerOffset&&!b.draped;z&&(d.uniforms.add(new l.FloatPassUniform("markerScale",e=>e.markerScale)),c.include(I.MarkerSizing,{space:R.LineMarkerSpace.World}));b.output===g.ShaderOutput.Depth&&
c.include(H.OutputDepth,b);c.include(F.ObjectAndLayerIdColor,b);M.addProjViewLocalOrigin(d,b);d.uniforms.add([new O.Matrix4PassUniform("inverseProjectionMatrix",(e,h)=>h.camera.inverseProjectionMatrix),new N.Float2PassUniform("nearFar",(e,h)=>h.camera.nearFar),new l.FloatPassUniform("miterLimit",e=>"miter"!==e.join?0:e.miterLimit),new r.Float4PassUniform("viewport",(e,h)=>h.camera.fullViewport)]);d.constants.add("LARGE_HALF_FLOAT","float",65500);c.attributes.add(m.VertexAttribute.POSITION,"vec3");
...
return posNdc;
  ...
  `);d.uniforms.add(new l.FloatPassUniform("pixelRatio",(e,h)=>h.camera.pixelRatio));d.code.add(a.glsl`
  void main(void) {
...
vReverseSegmentSDF = (isJoin && !isStartVertex) ? LARGE_HALF_FLOAT : (dot(pos.xy - segmentEnd, -segmentDir) * pos.w);`);b.stippleEnabled&&(b.draped?d.uniforms.add(new l.FloatPassUniform("worldToScreenRatio",(e,h)=>1/h.screenToPCSRatio)):d.code.add(a.glsl`vec3 segmentCenter = mix((auxpos2 + position) * 0.5, (position + auxpos1) * 0.5, isEndVertex);
...
float segmentLengthPseudoScreen = segmentLengthRender * discreteWorldToScreenRatio;`),d.uniforms.add(new l.FloatPassUniform("stipplePatternPixelSize",e=>w.computePixelSize(e))),d.code.add(a.glsl`
  ...
  `);b.wireframe?f.code.add(a.glsl`vec4 finalColor = vec4(1.0, 0.0, 1.0, 1.0);`):(k&&f.code.add(a.glsl`
    ...
    `),C?f.code.add(a.glsl`
      ...
      `):f.code.add(a.glsl`float stippleAlpha = getStippleAlpha();`),f.uniforms.add(new r.Float4PassUniform("intrinsicColor",e=>e.color)),b.output!==g.ShaderOutput.ObjectAndLayerIdColor&&f.code.add(a.glsl`discardByStippleAlpha(stippleAlpha, stippleAlphaColorDiscard);`),f.code.add(a.glsl`vec4 color = intrinsicColor * vColor;`),b.innerColorEnabled&&(f.uniforms.add(new r.Float4PassUniform("innerColor",e=>D.unwrapOr(e.innerColor,e.color))),f.uniforms.add(new l.FloatPassUniform("innerWidth",(e,h)=>e.innerWidth*
h.camera.pixelRatio)),f.code.add(a.glsl`float distToInner = abs(vLineDistance * gl_FragCoord.w) - innerWidth;
...
color = mix(color, vec4(innerColor.rgb, innerAlpha), innerAA);`)),f.code.add(a.glsl`vec4 finalColor = blendStipple(color, stippleAlpha);`),b.falloffEnabled&&(f.uniforms.add(new l.FloatPassUniform("falloff",e=>e.falloff)),f.code.add(a.glsl`finalColor.a *= pow(max(0.0, 1.0 - abs(vLineDistanceNorm * gl_FragCoord.w)), falloff);`)));f.code.add(a.glsl`
    ...
    ${b.output===g.ShaderOutput.Color&&b.transparencyPassType===Q.TransparencyPassType.Color?"fragColor \x3d premultiplyAlpha(fragColor);":""}
    ${b.output===g.ShaderOutput.Highlight?a.glsl`fragColor = vec4(1.0);`:""}
    ${b.output===g.ShaderOutput.Depth?a.glsl`outputDepth(linearDepth);`:""}
    ${b.output===g.ShaderOutput.ObjectAndLayerIdColor?a.glsl`outputObjectAndLayerIdColor();`:""}
  }
  `);return c}const S=Object.freeze(Object.defineProperty({__proto__:null,RIBBONLINE_NUM_ROUND_JOIN_SUBDIVISIONS:1,build:y},Symbol.toStringTag,{value:"Module"}));p.RIBBONLINE_NUM_ROUND_JOIN_SUBDIVISIONS=1;p.RibbonLine=S;p.build=y});