...
function(q,r,d,v,w,x,y,z,A,B,C,t,D,n,b,E,F,p,f){function u(a){const c=new E.ShaderBuilder,g=a.hasMultipassTerrain&&(a.output===d.ShaderOutput.Color||a.output===d.ShaderOutput.Alpha),{vertex:e,fragment:h}=c;t.addProjViewLocalOrigin(e,a);c.include(w.Transform,a);c.include(x.VertexColor,a);a.draped?e.uniforms.add(new n.FloatPassUniform("worldToScreenRatio",(m,k)=>1/k.screenToPCSRatio)):c.attributes.add(p.VertexAttribute.BOUNDINGRECT,"mat3");c.attributes.add(p.VertexAttribute.POSITION,"vec3");c.attributes.add(p.VertexAttribute.UVMAPSPACE,
"vec4");c.varyings.add("vpos","vec3");c.varyings.add("vuv","vec2");g&&c.varyings.add("depth","float");var l=a.style===f.Style.ForwardDiagonal||a.style===f.Style.BackwardDiagonal||a.style===f.Style.DiagonalCross;l&&e.code.add(b.glsl`
...
return accumulator;
    ...
    void main() {
      ...
      ${a.output===d.ShaderOutput.Color?b.glsl`fragColor = color; ${a.transparencyPassType===F.TransparencyPassType.Color?"fragColor \x3d premultiplyAlpha(fragColor);":""}`:""}
      ${a.output===d.ShaderOutput.Highlight?b.glsl`outputHighlight();`:""}
      ${a.output===d.ShaderOutput.Depth?b.glsl`outputDepth(linearDepth);`:""};
    }
  `);return c}function G(a){function c(g){return a.draped?b.glsl`coverage(vuv.${g}, texelSize)`:b.glsl`sampleAA(vuv.${g})`}switch(a.style){case f.Style.ForwardDiagonal:case f.Style.Horizontal:return c("y");case f.Style.BackwardDiagonal:case f.Style.Vertical:return c("x");case f.Style.DiagonalCross:case f.Style.Cross:return b.glsl`
        1.0 - (1.0 - ${c("x")}) * (1.0 - ${c("y")})
