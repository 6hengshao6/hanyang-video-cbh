...
a)}function ma(b){if(Array.isArray(b)){let a=[];for(let c=0;c<b.length;c++){let d=b[c].toString().split(".");for(let e=0;e<d.length;e++)a.push(d[e])}return a.join(".")}return b}function gc(b){return Array.isArray(b)?ma(b).split("."):b.toString().split(".")}function C(b,a,c){a=gc(a);for(let d=0;d<a.length;d++){if(!b)return;b=b[a[d]]}c&&(c.path=a.join("."));return b}function hc(b,a,c){let d=gc(a),e=d[d.length-1];if(1<d.length){for(a=0;a<d.length-1;a++)if(b=b[d[a]],!b)return;b[e]=c}else b[a]=c;return d.join(".")}
...
for(let g=0,h;g<a.args.length&&(h=a.args[g]);g++)h.literal||b._addPropertyEffect(h.rootProperty,c,{fn:d,info:e,trigger:h});f&&b._addPropertyEffect(a.methodName,c,{fn:d,info:e});return e}function tb(b,a,c,d,e){d=b._methodHost||b;let f=d[e.methodName];if(f)return b=b._marshalArgs(e.args,a,c),b===pa?pa:f.apply(d,b);e.dynamicFn||console.warn("method `"+e.methodName+"` not defined")}function pc(b){let a="";for(let c=0;c<b.length;c++)a+=b[c].literal||"";return a}function vb(b){var a=b.match(/([^\s]+?)\(([\s\S]*)\)/);
...
""),a=a.replace(K.mixinApply,"").replace(K.varApply,"")),(d=a.trim())&&(d="  "+d+"\n")}d&&(b.selector&&(c+=b.selector+" {\n"),c+=d,b.selector&&(c+="}\n\n"));return c}function Eb(b,a){if(!b)return"";"string"===typeof b&&(b=Gc(b));a&&ta(b,a);return Jc(b,Fb)}function Kc(b){!b.__cssRules&&b.textContent&&(b.__cssRules=Gc(b.textContent));return b.__cssRules||null}function ta(b,a,c,d){if(b){var e=!1,f=b.type;if(d&&f===Q.MEDIA_RULE){let g=b.selector.match(Ke);g&&(window.matchMedia(g[1]).matches||(e=!0))}f===
...
return a(c,e,d,b)}function Mc(b){if(void 0!==ua)return ua;if(void 0===b.__cssBuild){var a=b.getAttribute("css-build");if(a)b.__cssBuild=a;else{a:{a="template"===b.localName?b.content.firstChild:b.firstChild;if(a instanceof Comment&&(a=a.textContent.trim().split(":"),"css-build"===a[0])){a=a[1];break a}a=""}if(""!==a){const c="template"===b.localName?b.content.firstChild:b.firstChild;c.parentNode.removeChild(c)}b.__cssBuild=a}}return b.__cssBuild||""}function Le(b){if(b=Gb[b])b._applyShimCurrentVersion=
b._applyShimCurrentVersion||0,b._applyShimValidatingVersion=b._applyShimValidatingVersion||0,b._applyShimNextVersion=(b._applyShimNextVersion||0)+1}function Nc(b){return b._applyShimCurrentVersion===b._applyShimNextVersion}function Me(b){b._applyShimValidatingVersion=b._applyShimNextVersion;b._validating||(b._validating=!0,Ne.then(function(){b._applyShimCurrentVersion=b._applyShimNextVersion;b._validating=!1}))}function Oc(){va=document.documentElement.getAttribute("dir");va=document.documentElement.getAttribute("dir");
for(let a=0;a<wa.length;a++){var b=wa[a];b.__autoDirOptOut||b.setAttribute("dir",va)}}function Pc(){document.body.removeAttribute("unresolved")}function xa(b,a,c){return{index:b,removed:a,addedCount:c}}function Qc(b,a,c,d,e,f){var g=0,h=0,k=Math.min(c-a,f-e);if(0==a&&0==e)a:{for(g=0;g<k;g++)if(b[g]!==d[g])break a;g=k}if(c==b.length&&f==d.length){h=k-g;k=b.length;for(var l=d.length,m=0;m<h&&Oe(b[--k],d[--l]);)m++;h=m}a+=g;e+=g;c-=h;f-=h;if(0==c-a&&0==f-e)return[];if(a==c){for(c=xa(a,[],0);e<f;)c.removed.push(d[e++]);
return[c]}if(e==f)return[xa(a,[],c-a)];g=a;h=e;f=f-h+1;k=c-g+1;c=Array(f);for(l=0;l<f;l++)c[l]=Array(k),c[l][0]=l;for(l=0;l<k;l++)c[0][l]=l;for(l=1;l<f;l++)for(m=1;m<k;m++)if(b[g+m-1]===d[h+l-1])c[l][m]=c[l-1][m-1];else{var n=c[l-1][m]+1;let p=c[l][m-1]+1;c[l][m]=n<p?n:p}f=c.length-1;g=c[0].length-1;h=c[f][g];for(b=[];0<f||0<g;)0==f?(b.push(2),g--):0==g?(b.push(3),f--):(k=c[f-1][g-1],l=c[f-1][g],m=c[f][g-1],n=l<m?l<k?l:k:m<k?m:k,n==k?(k==h?b.push(0):(b.push(1),h=k),f--,g--):n==l?(b.push(3),f--,h=
l):(b.push(2),g--,h=m));b.reverse();c=void 0;f=[];for(g=0;g<b.length;g++)switch(b[g]){case 0:c&&(f.push(c),c=void 0);a++;e++;break;case 1:c||(c=xa(a,[],0));c.addedCount++;a++;c.removed.push(d[e]);e++;break;case 2:c||(c=xa(a,[],0));c.addedCount++;a++;break;case 3:c||(c=xa(a,[],0)),c.removed.push(d[e]),e++}c&&f.push(c);return f}function Oe(b,a){return b===a}function ca(b){return"slot"===b.localName}function Pe(b,a){for(let c=0;c<a.length;c++){let d=a[c];b[d]=function(){return this.node[d].apply(this.node,
...
d.substring(6),e[d])}}function We(b,a){return function(c,d,e){a.call(c.__templatizeOwner,c,d,e[d])}}function Xe(){return function(b,a,c){b.__dataHost._setPendingPropertyOrPath("_host_"+a,c[a],!0,!0)}}function za(b,a,c){if(da&&!Yc(b))throw Error("strictTemplatePolicy: template owner not trusted");c=c||{};if(b.__templatizeOwner)throw Error("A \x3ctemplate\x3e can only be templatized once");b.__templatizeOwner=a;let d=(a?a.constructor:R)._parseTemplate(b);var e=d.templatizeInstanceClass;if(!e){e=c;var f=
e.mutableData?Ye:R;za.mixin&&(f=za.mixin(f));var g=class extends f{};g.prototype.__templatizeOptions=e;g.prototype._bindTemplate(b);f=g;var h=d.hostProps||{};for(var k in e.instanceProps){delete h[k];var l=e.notifyInstanceProp;l&&f.prototype._addPropertyEffect(k,f.prototype.PROPERTY_EFFECT_TYPES.NOTIFY,{fn:We(k,l)})}if(e.forwardHostProp&&b.__dataHost)for(var m in h)d.hasHostProps||(d.hasHostProps=!0),f.prototype._addPropertyEffect(m,f.prototype.PROPERTY_EFFECT_TYPES.NOTIFY,{fn:Xe()});e=g;d.templatizeInstanceClass=
...
b}},validateCallback:{get(){return Aa},set(b){let a=!1;Aa||(a=!0);Aa=b;a&&this.enqueueDocumentValidation()}}});const Nb=/(?:^|[;\s{]\s*)(--[\w-]*?)\s*:\s*(?:((?:'(?:\\'|.)*?'|"(?:\\"|.)*?"|\([^)]*?\)|[^};{])+)|\{([^}]*)\}(?:(?=[;\s}])|$))/gi,Wa=/(?:^|\W+)@apply\s*\(?([^);\n]*)\)?/gi,Ke=/@media\s(.*)/,La=!(window.ShadyDOM&&window.ShadyDOM.inUse);let mb,ua;window.ShadyCSS&&void 0!==window.ShadyCSS.cssBuild&&(ua=window.ShadyCSS.cssBuild);const bd=!(!window.ShadyCSS||!window.ShadyCSS.disableRuntime);
...
b[e+1]},b[0]);return new Pb(a,kd)};(window.litElementVersions||(window.litElementVersions=[])).push("2.5.1");let ld=0;const Qb={},Da=(b,a,c)=>{const d=c&&c.moduleId||`custom-style-module-${ld++}`;Array.isArray(a)||(a=a?[a]:[]);a.forEach(f=>{if(!(f instanceof Pb))throw Error("An item in styles is not of type CSSResult. Use `unsafeCSS` or `css`.");if(!Qb[f]){const g=document.createElement("dom-module");g.innerHTML=`\n        <template>\n          <style>${f.toString()}</style>\n        </template>\n      `;
...
qf=/[A-Z]/,ud=(b,a,c,d,e)=>{var f=e?J(b):b;if(a=a[f])for(f=0;f<a.length;f++){var g=a[f];if(g.info.lastRun!==oa&&(!e||sb(b,g.trigger))){g.info.lastRun=oa;g=g.info;var h=c,k=d;let l=0,m=h.length-1,n=-1;for(;l<=m;){const p=l+m>>1,r=k.get(h[p].methodInfo)-k.get(g.methodInfo);if(0>r)l=p+1;else if(0<r)m=p-1;else{n=p;break}}0>n&&(n=m+1);h.splice(n,0,g)}}},ue=[],vd=RegExp("(\\[\\[|{{)\\s*(?:(!)\\s*)?((?:[a-zA-Z_$][\\w.:$\\-*]*)\\s*(?:\\(\\s*(?:(?:(?:((?:[a-zA-Z_$][\\w.:$\\-*]*)|(?:[-+]?[0-9]*\\.?[0-9]+(?:[eE][-+]?[0-9]+)?)|(?:(?:'(?:[^'\\\\]|\\\\.)*')|(?:\"(?:[^\"\\\\]|\\\\.)*\")))\\s*)(?:,\\s*(?:((?:[a-zA-Z_$][\\w.:$\\-*]*)|(?:[-+]?[0-9]*\\.?[0-9]+(?:[eE][-+]?[0-9]+)?)|(?:(?:'(?:[^'\\\\]|\\\\.)*')|(?:\"(?:[^\"\\\\]|\\\\.)*\")))\\s*))*)?)\\)\\s*)?)(?:]]|}})",
...
e){e=e||this._bindTemplate(d,!0);Ea.push(this);d=super._stampTemplate(d,e);Ea.pop();e.nodeList=d.nodeList;if(!e.wasPreBound){var f=e.childNodes=[];for(var g=d.firstChild;g;g=g.nextSibling)f.push(g)}d.templateInfo=e;let {nodeList:h,nodeInfoList:k}=e;if(k.length)for(let m=0;m<k.length;m++){let n=h[m],p=k[m].bindings;if(p)for(let r=0;r<p.length;r++){let v=p[r];f=n;g=v;if(g.isCompound){let B=f.__dataCompoundStorage||(f.__dataCompoundStorage={});var l=g.parts;let H=Array(l.length);for(let T=0;T<l.length;T++)H[T]=
...
window.ShadyDOM&&window.ShadyDOM.noPatch?window.ShadyDOM.composedPath:b=>b.composedPath&&b.composedPath()||[],ra={},aa=[];Bb({name:"downup",deps:["mousedown","touchstart","touchend"],flow:{start:["mousedown","touchstart"],end:["mouseup","touchend"]},emits:["down","up"],info:{movefn:null,upfn:null},reset:function(){ba(this.info)},mousedown:function(b){if(Z(b)){var a=P(b),c=this;wc(this.info,function(d){Z(d)||(sa("up",a,d),ba(c.info))},function(d){Z(d)&&sa("up",a,d);ba(c.info)});sa("down",a,b)}},touchstart:function(b){sa("down",
P(b),b.changedTouches[0],b)},touchend:function(b){sa("up",P(b),b.changedTouches[0],b)}});Bb({name:"track",touchAction:"none",deps:["mousedown","touchstart","touchmove","touchend"],flow:{start:["mousedown","touchstart"],end:["mouseup","touchend"]},emits:["track"],info:{x:0,y:0,state:"start",started:!1,moves:[],addMove:function(b){2<this.moves.length&&this.moves.shift();this.moves.push(b)},movefn:null,upfn:null,prevent:!1},reset:function(){this.info.state="start";this.info.started=!1;this.info.moves=
...
class Dd extends Cd(yf(Ub(Ga(M)))){static get template(){return Fa`
      <style>
        :host {
          display: inline-block;
        }
...
D.prototype.transformCustomStyle=D.prototype.transformCustomStyle;D.prototype.transformRules=D.prototype.transformRules;D.prototype.transformRule=D.prototype.transformRule;D.prototype.transformTemplate=D.prototype.transformTemplate;D.prototype._separator="_-_";Object.defineProperty(D.prototype,"invalidCallback",{get(){return eb},set(b){eb=b}});const Gb={},Ne=Promise.resolve(),Ha=new D;class Df{constructor(){this.customStyleInterface=null;Ha.invalidCallback=Le}ensure(){!this.customStyleInterface&&
window.ShadyCSS.CustomStyleInterface&&(this.customStyleInterface=window.ShadyCSS.CustomStyleInterface,this.customStyleInterface.transformCallback=b=>{Ha.transformCustomStyle(b)},this.customStyleInterface.validateCallback=()=>{requestAnimationFrame(()=>{this.customStyleInterface.enqueued&&this.flushCustomStyles()})})}prepareTemplate(b,a){this.ensure();""===Mc(b)&&(Gb[a]=b,a=Ha.transformTemplate(b,a),b._styleAst=a)}flushCustomStyles(){this.ensure();if(this.customStyleInterface){var b=this.customStyleInterface.processStyles();
...
nativeCss:Fb,nativeShadow:La,cssBuild:ua,disableRuntime:bd};a&&(window.ShadyCSS.CustomStyleInterface=a)}window.ShadyCSS.ApplyShim=Ha;const Ef=/:host\(:dir\((ltr|rtl)\)\)/g,Ff=/([\s\w-#\.\[\]\*]*):dir\((ltr|rtl)\)/g,Gf=/:dir\((?:ltr|rtl)\)/,Gd=!(!window.ShadyDOM||!window.ShadyDOM.inUse),wa=[];let Ia=null,va="";const Hf=F(b=>{Gd||Ia||(va=document.documentElement.getAttribute("dir"),Ia=new MutationObserver(Oc),Ia.observe(document.documentElement,{attributes:!0,attributeFilter:["dir"]}));const a=rd(b);
...
(Ia&&Ia.takeRecords().length&&Oc(),wa.push(this),this.__autoDirOptOut||this.setAttribute("dir",va))}disconnectedCallback(){a.prototype.disconnectedCallback&&super.disconnectedCallback();if(this.constructor.__activateDir){const d=wa.indexOf(this);-1<d&&wa.splice(d,1)}}}c.__activateDir=!1;return c});"interactive"===document.readyState||"complete"===document.readyState?Pc():window.addEventListener("DOMContentLoaded",Pc);let N=class{static getFlattenedNodes(b){const a=q(b);return ca(b)?a.assignedNodes({flatten:!0}):
...
R.prototype.__dataHost;R.prototype.__templatizeOptions;R.prototype._methodHost;R.prototype.__templatizeOwner;R.prototype.__hostProps;const Ye=ac(R);let ad=!1;const Kf=Ga(Ld(ab(HTMLElement)));class Lf extends Kf{static get observedAttributes(){return["mutable-data"]}constructor(){super();if(da)throw Error("strictTemplatePolicy: dom-bind not allowed");this.__children=this.$=this.root=null}attributeChangedCallback(b,a,c,d){this.mutableData=!0}connectedCallback(){Lb()||(this.style.display="none");this.render()}disconnectedCallback(){this.__removeChildren()}__insertChildren(){q(q(this).parentNode).insertBefore(this.root,
...
a&&(this.$.table.scrollLeft+=Math.round(c.left-a));c.right>f&&(this.$.table.scrollLeft+=Math.round(c.right-f))}}_elementMatches(a,c){return a.matches?a.matches(c):-1!==Array.from(a.parentNode.querySelectorAll(c)).indexOf(a)}},kg=b=>class extends Ga(b){static get properties(){return{columnReorderingAllowed:{type:Boolean,value:!1},_orderBaseScope:{type:Number,value:1E7}}}static get observers(){return["_updateOrders(_columnTree, _columnTree.*)"]}ready(){super.ready();Ab(this,"track",this._onTrackEvent);
...
this._previousHidden=a}};class hb extends Sd(Xb(M)){static get is(){return"vaadin-grid-column"}static get properties(){return{width:{type:String,value:"100px"},flexGrow:{type:Number,value:1},renderer:Function,path:{type:String},autoWidth:{type:Boolean,value:!1},_bodyTemplate:{type:Object},_cells:Array}}}customElements.define(hb.is,hb);Da("vaadin-grid",Ca`
  ...
  `,{moduleId:"vaadin-grid-styles"});try{document.createEvent("TouchEvent");var Td=!0}catch(b){Td=!1}const lg=Td;class Ud extends Cd(Ub($f(Yf(ag(Xf(eg(fg(gg(dg(jg(Vf(cg(kg(Zf(bg(ig(hg(Uf)))))))))))))))))){static get template(){return Fa`
      <div
        id="scroller"
        safari$="[[_safari]]"
        ios$="[[_ios]]"
...
this._grid._updateColumnTree()})});this._observer.flush()}_isColumnElement(b){return b.nodeType===Node.ELEMENT_NODE&&/\bcolumn\b/.test(b.localName)}}customElements.define(Vd.is,Vd);class Wd extends hb{static get template(){return Fa`
...
!Array.isArray(this._grid.items)}}customElements.define(Wd.is,Wd);Da("vaadin-grid-sorter",Ca`
    :host {
...
document.head.appendChild(Xd.content);class Yd extends Ub(Xb(M)){static get template(){return Fa`
      <style>
        :host {
          display: inline-flex;
          cursor: pointer;
...
"asc"===this.direction?"desc":"desc"===this.direction?null:"asc")}}customElements.define(Yd.is,Yd);class Zd extends hb{static get template(){return Fa`
      <template class="header" id="headerTemplate">
        <vaadin-grid-sorter path="[[path]]" direction="{{direction}}">[[_getHeader(header, path)]]</vaadin-grid-sorter>
      </template>
    `}static get is(){return"vaadin-grid-sort-column"}static get properties(){return{path:String,direction:{type:String,notify:!0}}}_prepareHeaderTemplate(){const b=this._prepareTemplatizer(this.$.headerTemplate);b.templatizer.dataHost=this;return b}_getHeader(b,a){return b||this._generateHeader(a)}}customElements.define(Zd.is,Zd);const $d={selectionColumn:!0,columnMenus:!0};let x=function(b){function a(d,e){d=c.call(this,d,e);d._grid=null;d._columnElements=[];d._isDataLoaded=!1;d.itemIdPath=null;
d.messages=null;d.viewModel=new ee;d.visibleElements={...$d};return d}ib._inherits(a,b);var c=ib._createSuper(a);b=a.prototype;b.initialize=function(){this.handles.add([this.columns.on("change",()=>this.onColumnChange()),Ka.watch(()=>this.viewModel?.size,()=>this._updateGridSize()),Ka.watch(()=>this.store?.state,(d,e)=>{"ready"!==d||"loaded"!==e&&"error"!==e||this.refreshPageCache()}),Ka.on(()=>this.highlightIds,"change",d=>{const {added:e,removed:f}=d;d=e.map(h=>this.store?.getLocalItemByKey(h)||
{objectId:h});const g=f.map(h=>this.store?.getLocalItemByKey(h)||{objectId:h});this._deselectRows(g);this._selectRows(d)}),Ka.on(()=>this._grid?.$?.table,"scroll",()=>this.viewModel?.closeColumnMenus())])};b.destroy=function(){this.handles.removeAll();this.resetColumns();this.columns?.destroy()};b.resetColumns=function(){this.columns.items.forEach(d=>d.destroy());this.columns.removeAll()};b.castVisibleElements=function(d){return{...$d,...d}};b.render=function(){const {columnMenus:d}=this.visibleElements;
return Y.tsx("div",{bind:this,class:this.classes("esri-grid","esri-widget",{["esri-grid--no-column-menu"]:!d})},Y.tsx("div",{bind:this,class:"esri-grid__content"},this.renderGrid()))};b.renderGrid=function(){return Y.tsx("vaadin-grid",{_safari:!1,afterCreate:this._afterGridCreate,afterUpdate:this._afterGridUpdate,bind:this,cellClassNameGenerator:this.cellClassNameGenerator,class:"esri-grid__grid",columnReorderingAllowed:this.columnReorderingEnabled,dataProvider:this.dataProvider,id:`${this.id}_grid`,
itemIdPath:this.itemIdPath,multiSort:this.multiSortEnabled,pageSize:this.pageSize,ref:"grid",rowDetailsRenderer:this.rowDetailsRenderer,size:this.size},this.renderAllColumns())};b.renderAllColumns=function(){return"disabled"!==this.viewModel.state&&this.columns?.length?[this.renderSelectionColumn(),this.renderColumns()]:null};b.renderSelectionColumn=function(){return Y.tsx("vaadin-grid-selection-column",{_selectAllHidden:!0,selectAll:!1,bind:this,hidden:!this.visibleElements.selectionColumn,sortable:!1,
frozen:!fe.isRTL(this.container)})};b.renderColumns=function(){return this.columns.items.map((d,e)=>"columns"in d?this.renderGroupColumn(d,e):Y.tsx("vaadin-grid-column",{...this.getColumnProps(d,e)})).filter(ka.isSome)};b.renderGroupColumn=function(d,e){const f=this.getColumnProps(d,e);if(f.hidden||!d.columns)return null;d=d.columns.filter(({hidden:g})=>!g);return d.length?Y.tsx("vaadin-grid-column-group",{...f},d.map(g=>Y.tsx("vaadin-grid-column",{...this.getColumnProps(g,e)}))):null};b.getColumnProps=
function(d,e){const {id:f}=this,{autoWidth:g,direction:h,flexGrow:k,frozen:l,header:m,hidden:n,path:p,resizable:r,textAlign:v,width:B}=d;d=`${f}_${m}_${ka.isSome(e)?e:p}`;return{autoWidth:g,direction:h,flexGrow:k,frozen:l,header:m,hidden:n,key:d,path:p,resizable:r,"text-align":v,width:"number"===typeof B?`${B}px`:B,bind:this,afterCreate:this._afterColumnCreate,afterRemoved:this._afterColumnRemoved}};b.clearSelection=function(){this._clearSelection();this.scheduleRender()};b.clearSort=function(){let d=
!1;this._grid&&(this._grid._sorters?.length&&(this._grid._sorters.forEach(e=>{e._order=null;e.direction=null}),d=!0),this.columns?.length&&this.columns.some(e=>!!e.direction)&&(this.columns.forEach(e=>e.direction=null),d=!0),d&&(this.notifyChange("sortOrders"),this.scheduleRender()))};b.findColumn=function(d){this.viewModel.findColumn(d)};b.generateCellClassNames=function(){this._grid?.generateCellClassNames()};b.getSlotElementByName=function(d){return this._grid?.shadowRoot?.querySelector(`slot[name='${d}']`)??
null};b.hideColumn=function(d){this.viewModel?.hideColumn(d);this.scheduleRender()};b.notifyResize=function(){this._grid?.notifyResize()};b.recalculateColumnWidths=function(){this._grid?.recalculateColumnWidths()};b.reset=async function(){this._isDataLoaded=!1;this._clearSelection();await this.store?.reset();this.scrollToTop()};b.refreshPageCache=function(){this._grid?.clearCache()};b.selectRows=function(d){d=d?.filter(f=>!this.highlightIds.includes(f.objectId));const e=d.map(f=>f.objectId);e.length&&
(this.highlightIds.addMany(e),this._selectRows(d))};b.deselectRows=function(d){const e=d?.map(f=>f.objectId)||[];e.length&&(this.highlightIds.removeMany(e),this._deselectRows(d))};b.showColumn=function(d){this.viewModel?.showColumn(d);this.scheduleRender()};b.sort=function({path:d,direction:e}){this.viewModel?.sortColumn(d,e);this.notifyChange("sortOrders")};b.scrollToIndex=function(d){this._grid?.scrollToIndex(d)};b.scrollToTop=function(){this._grid?.scrollToIndex(0)};b.isEditingActive=function(){return!!this._getActiveEditInfo().length};
b.onColumnChange=function(){this._syncColumnRenderers()};b._afterGridCreate=function(d){const e=this._grid=d;e.setAttribute("theme","compact column-borders");customElements.whenDefined("vaadin-grid").then(()=>this._addGridEventListeners());e.__updateSorter=f=>{var g=e._sorters;const h=!g.includes(f);if(f.direction||!h)f._order=null,e.multiSort?(e._removeArrayItem(g,f),f.direction&&(ka.isSome(f._initialOrder)?(h?g[f._initialOrder]=f:g.splice(f._initialOrder,0,f),f._initialOrder=null):g.unshift(f)),
e.__updateSortOrders()):f.direction&&(g=g.filter(k=>k!==f),e._sorters=[f],g.forEach(k=>{k._order=null;k.direction=null}))}};b._afterGridUpdate=function(d){this._grid||(this._grid=d)};b._afterColumnCreate=function(d){this._applyRenderersToColumnElement(d);this._columnElements.push(d)};b._afterColumnRemoved=function(d){d=this._columnElements.indexOf(d,0);-1<d&&this._columnElements.splice(d,1)};b._syncColumnRenderers=function(){this._columnElements.forEach(d=>{this._applyRenderersToColumnElement(d)})};
b._updateGridSize=function(){this._grid&&(this._grid.size=this.size||0,this.scheduleRender())};b._getActiveEditInfo=function(){return!this.columns||this.columns.length?[]:this.columns.filter(d=>"activeEditInfo"in d).map(d=>d.activeEditInfo).items};b._addGridEventListeners=function(){const d=this._grid;ka.assertIsSome(d);this.handles.add([Ja.on(d,"click",e=>this._onRowClick(e)),Ja.on(d,"selected-items-changed",e=>this._onSelectionChange(e)),Ja.on(d,"sorter-changed",()=>this.notifyChange("sortOrders")),
Ja.on(d,"loading-changed",e=>{this._isDataLoaded||!1!==e.detail.value||(this._isDataLoaded=!0,this._selectRowsOnLoad())})])};b._onRowClick=function(d){var e=this._grid;ka.assertIsSome(e);e=e.getEventContext(d);const {item:f,section:g}=e;f&&g&&"details"!==g&&"header"!==g&&this.emit("row-click",{context:e,native:d})};b._clearSelection=function(){this._grid&&(this.highlightIds.removeAll(),this._grid.selectedItems=[],this._grid.render())};b._selectRowsOnLoad=function(){const d=this._grid;if(d){var e=
d.selectedItems.slice();(this.highlightIds?.filter(f=>0>e.findIndex(g=>g.objectId===f))).forEach(f=>d.selectedItems.push(this.store?.getLocalItemByKey(f)||{objectId:f}));d.render()}};b._selectRows=function(d){const e=this._grid;if(e){var f=e.selectedItems.slice();d.filter(g=>0>f.findIndex(h=>h.objectId===g.objectId)).forEach(g=>e.selectedItems.push(g));e.render()}};b._deselectRows=function(d){const e=this._grid;if(e){var f=e.selectedItems.slice();d.forEach(g=>{const h=f.findIndex(k=>k.objectId===
g.objectId);-1<h&&f.splice(h,1)});e.selectedItems=f;e.render()}};b._onSelectionChange=function(d){if("selectedItems.splices"===d.detail.path){const {removed:e,object:f}=d.detail.value.indexSplices[0];d=f.filter(h=>!this.highlightIds.includes(h.objectId)).map(h=>h.objectId);const g=e.filter(h=>this.highlightIds.includes(h.objectId)).map(h=>h.objectId);this.highlightIds.removeMany(g);this.highlightIds.addMany(d)}};b._applyRenderersToColumnElement=function(d){const e=this.viewModel.findColumn(d.path);
if(e)try{const {renderFunction:f,footerRenderFunction:g,headerRenderFunction:h}=e;f&&"renderer"in d&&(d.renderer=(k,l,m)=>f({root:k,column:l,rowData:m}));g&&(d.footerRenderer=(k,l)=>g({root:k,column:l}));h&&(d.headerRenderer=(k,l)=>h({root:k,column:l}))}catch(f){}};ib._createClass(a,[{key:"cellClassNameGenerator",get:function(){return this.viewModel.cellClassNameGenerator},set:function(d){this.viewModel.cellClassNameGenerator=d}},{key:"columns",get:function(){return this.viewModel.columns},set:function(d){this.viewModel.columns=
